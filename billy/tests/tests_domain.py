...
def test_start_session_time():
    ...
    """
    assert time.localtime() == domain.WorkSession(client='B', project='P').start_time # hopefully fast enough to catch the time !
...
def test_end_session_time():
    ...
    """
    assert time.localtime() == domain.WorkSession(client='B', project='P').end_session().end_time # hopefully fast enough !
